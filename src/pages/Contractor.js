...
function Contractor() {
  ...
  useEffect(() => {
    ...
    axios
      .get(base_url + `/con_add?${mess_id ? "mess_id=" + mess_id : ""}`)
      .then((response) => {
        response.data[0] != undefined
          ? setData((prev) => ({
              title: mess_id ? `Contractor detail` : `Contractor detail`,
              rows: response.data.map((ele) => ({
                "contractor Id": ele["contractor Id"],
                "Contractor Name": ele["Contractor Name"],
                "Mess Id": ele["Mess Id"],
                "HQ Office Number": ele["HQ Office Number"],
                "HQ Street Number": ele["HQ Street Number"],
                "HQ Street Name": ele["HQ Street Name"],
                "HQ city": ele["HQ city"],
                "HQ State": ele["HQ State"],
                "HQ Pincode": ele["HQ Pincode"],
              })),
            }))
          : setData({ title: "", rows: [{}] });
