...
function Balance_sheet() {
  ...
  useEffect(() => {
    ...
    axios
      .get(base_url + `/balance_sheet?${mess_id ? "mess_id=" + mess_id : ""}`)
      .then((response) => {
        response.data[0] != undefined
          ? setData((prev) => ({
              title: mess_id ? `Balance Sheet` : `Balance Sheet`,
              rows: response.data.map((ele) => ({
                balance_id: ele["balance_id"],
                mess_id: ele["mess_id"],
                month: ele["month"],
                year: ele["year"],
                from_guest: ele["from_guest"],
                from_student: ele["from_student"],
                miscellaneous: ele["miscellaneous"],
                to_employee: ele["to_employee"],
                to_vendor: ele["to_vendor"],
              })),
            }))
          : setData({ title: "", rows: [{}] });
